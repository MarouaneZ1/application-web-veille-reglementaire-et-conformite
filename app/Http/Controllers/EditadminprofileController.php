...
/*   public function society(){
  ...
  public function admin(){
    $info = DB::table('users')->where('id','=', session('LoggedUser'))->select('email','first_name','last_name','Tel','CIN')->get();
    if($info){
      return view('user_dashboard.profile.Admin',['info'=>$info]); 
    }
    return redirect('/adminprofile')->with('failrespo','Erreur ! veuillez réessayer.. ');
  }
 ...
 */
  ...
  public function editadmin(Request $r){
    ...
    $r->validate($rules, $customMessages,$attr);
    $return=DB::table('users')->where('id',session('LoggedUser'))->update([
      'last_name'=>$r->Nom,
      'first_name'=>$r->Prenom,
      'email'=>$r->Email,
      'Tel'=>$r->Telephone,
    ...
    if($return){
      return redirect('/Adminprofile')->with('successresp',"les modifications ont bien été enregistrées");
    }
    return redirect('/Adminprofile')->with('failresp','Vous avez rien modifié ! veuillez réessayer.. ');
